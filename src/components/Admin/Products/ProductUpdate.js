...
export default function ProductUpdate() {
  ...
  // converted ages
  const ageOptionsConverted = ages.map((age) => {
    return {
      value: age,
      label: age,
    };
  ...
  // converted colors
  const colorConverted = colors?.map((color) => {
    return {
      value: color?.name,
      label: color?.name,
    };
            ...
            <form className="space-y-6" onSubmit={handleOnSubmit}>
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(item) => handleAgeChange(item)}
                />
              </div>
              {/* Select category */}
              <div>
